...
import { validateTwilioSignature, getVoiceGatewayTwiML } from '../../api/voice/twilio-inbound';
...
describe('Twilio Webhook & Voice Gateway Routing', () => {
  ...
  it('rejects invalid HMAC signatures', () => {
    const isValid = validateTwilioSignature(token, 'invalid_signature', url, { CallSid: 'CA123' });
    expect(isValid).toBe(false);
  });

  it('generates valid TwiML for Cartesia / ElevenLabs streaming websocket', () => {
    const twiml = getVoiceGatewayTwiML({ provider: 'cartesia', voiceId: 'sonic-1' }, 'Hello world');
    expect(twiml).toContain('<Say voice="Polly.Amy">Hello world</Say>');
    expect(twiml).toContain('<Stream url="wss://api.cartesia.ai/tts/websocket">');
...
describe('Sub-account Wallet Metering & Invoice Generation', () => {
  ...
  it('generates monthly invoice record with tenant isolation', async () => {
    const inv = await WalletManager.generateMonthlyInvoice('agency-alpha', 'sub-123', '2026-07', 4500);
...
describe('pgvector RAG Chunking & Ingestion Safety', () => {
  ...
  it('rejects payloads exceeding the 10MB memory limit', async () => {
    ...
    const res = await RagIngestionQueue.processIngestJob(
      { id: 'job-1', botId: 'bot-1', sourceType: 'pdf', sourceUrl: 'https://test.com/doc.pdf', maxChunkSize: 800 },
      oversized
    );
