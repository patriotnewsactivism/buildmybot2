...
export async function leadFollowupsHandler(
  ...
  const listResp = await sbFetch(
    `leads?replied_at=is.null&follow_up_sent_at=is.null&created_at=lte.${encodeURIComponent(cutoff)}&select=id,name,email,created_at,status,source_bot_id&order=created_at.asc`,
  );
  if (!listResp.ok) {
