...
import {
  SMS_PLANS,
  SMS_REGISTRATION_FEE_USD,
  type SmsPlan,
} from '../../shared/sms.js';
import { accountFor } from './runtime.js';
import { SmsError, type SmsUser, db, filter, rpc, scoped } from './store.js';
...
export async function stripe(
  ...
  if (!process.env.STRIPE_SECRET_KEY)
    throw new SmsError(503, 'Billing is not configured');
  ...
  if (!response.ok)
    throw new SmsError(502, `Billing operation failed (${response.status})`);
...
export async function createSmsCheckout(user: SmsUser, plan: SmsPlan) {
  ...
  if (account.subscription_id)
    throw new SmsError(
      409,
      'Manage the existing SMS subscription through billing before creating another',
    );
  if (!account.business_name)
    throw new SmsError(400, 'Save the business name and timezone first');
  const priceId = process.env[`STRIPE_PRICE_${plan}`];
  if (!priceId)
    throw new SmsError(503, 'This SMS plan is not configured for checkout');
  const price = await stripe(`/prices/${encodeURIComponent(priceId)}`);
  if (
    !price.active ||
    price.currency !== 'usd' ||
    price.unit_amount !== SMS_PLANS[plan].price * 100 ||
    price.recurring?.interval !== 'month' ||
    (price.recurring?.interval_count || 1) !== 1
  )
    throw new SmsError(
      503,
      'The configured billing price does not match the published SMS plan',
    );
  ...
  if (!feePriceId)
    throw new SmsError(
      503,
      'The SMS registration fee is not configured for checkout',
    );
  const feePrice = await stripe(`/prices/${encodeURIComponent(feePriceId)}`);
  if (
    !feePrice.active ||
    feePrice.currency !== 'usd' ||
    feePrice.unit_amount !== SMS_REGISTRATION_FEE_USD * 100 ||
    feePrice.recurring != null
  )
    throw new SmsError(
      503,
      'The configured registration-fee price does not match the published amount',
    );
  ...
  const session = await stripe(
    ...
    'POST',
    {
      ...
      'line_items[1][price]': feePriceId,
      'line_items[1][quantity]': '1',
      success_url: `${base}/app/sms-marketing?payment=complete`,
      cancel_url: `${base}/app/sms-marketing?payment=cancelled`,
      'metadata[type]': 'sms',
      'metadata[userId]': user.id,
      'metadata[tenantKey]': user.tenant,
      'subscription_data[metadata][type]': 'sms',
      'subscription_data[metadata][userId]': user.id,
      'subscription_data[metadata][tenantKey]': user.tenant,
      'subscription_data[metadata][planKey]': plan,
...
export async function handleSmsBillingEvent(
  event: StripeObject,
): Promise<boolean> {
  ...
  if (event.type.startsWith('customer.subscription.')) subscription = object;
    ...
    if (id)
      subscription = await stripe(
        `/subscriptions/${encodeURIComponent(typeof id === 'string' ? id : id.id)}`,
      );
  }
  if (subscription?.metadata?.type !== 'sms')
    return object.metadata?.type === 'sms';
  const tenant = subscription.metadata.tenantKey;
  if (typeof tenant !== 'string')
    throw new Error('SMS subscription missing tenant');
  const a = await accountFor(tenant);
  if (a.user_id !== subscription.metadata.userId)
    throw new Error('SMS subscription identity mismatch');
  if (a.subscription_id && a.subscription_id !== subscription.id)
    throw new Error('Duplicate SMS subscription requires reconciliation');
  if (event.type === 'invoice.paid') {
    const plan = subscription.metadata.planKey as SmsPlan;
    if (!(plan in SMS_PLANS)) throw new Error('Unknown SMS plan');
    const item = subscription.items?.data?.find(
      (i: StripeObject) => i.price?.id === process.env[`STRIPE_PRICE_${plan}`],
    );
    ...
    const expectedMinorUnits =
      (SMS_PLANS[plan].price +
        (isFirstInvoice ? SMS_REGISTRATION_FEE_USD : 0)) *
      100;
    if (
      !item ||
      object.paid !== true ||
      object.amount_paid < expectedMinorUnits ||
      object.currency !== 'usd'
    )
      throw new Error(
        isFirstInvoice
          ? 'SMS provisioning requires the first month plus the registration fee to be fully paid'
          : 'SMS provisioning requires a fully paid month',
      );
    const start =
    ...
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start)
      throw new Error('Invalid paid SMS period');
    await rpc('sms_apply_payment', {
      p_tenant: tenant,
    ...
    ['unpaid', 'canceled', 'paused'].includes(subscription.status)
  ) {
...
async function recordSmsOverageCommission(
  ...
  const users = await db<Array<{ email: string }>>(
    `users?${filter({ id: `eq.${userId}`, select: 'email', limit: '1' })}`,
  ).catch(() => []);
  const email = users[0]?.email;
  if (!email) return;

  const partnerClients = await db<Array<{ id: string; partner_id: string }>>(
    `partner_clients?${filter({ client_email: `eq.${email}`, status: 'eq.active', select: 'id,partner_id', limit: '1' })}`,
  ).catch(() => []);
  if (partnerClients[0]) {
    ...
    >(
      `partners?${filter({ id: `eq.${partnerClients[0].partner_id}`, status: 'eq.active', select: 'id,commission_rate,total_earned,pending_payout', limit: '1' })}`,
    ).catch(() => []);
    const partner = partners[0];
    if (partner) {
      const rawCommission = +(
        overageRevenueUsd * (partner.commission_rate || 0.15)
      ).toFixed(2);
      const { cappedCommissionUsd, wasCapped } =
        applySmsOverageCommissionSafeguard(overageRevenueUsd, rawCommission);
      if (cappedCommissionUsd > 0) {
        ...
        await db('partner_payouts', 'POST', [
          {
            partner_id: partner.id,
            amount: cappedCommissionUsd,
            status: 'pending',
          },
        ]).catch(() => {});
        if (wasCapped)
          console.warn(
            `[sms-billing] overage commission capped for partner ${partner.id} on invoice ${invoiceId}`,
          );
      }
      return;
    }
  ...
  const resellerClients = await db<Array<{ id: string; reseller_id: string }>>(
    `reseller_clients?${filter({ client_email: `eq.${email}`, status: 'eq.active', select: 'id,reseller_id', limit: '1' })}`,
  ).catch(() => []);
  if (resellerClients[0]) {
    ...
    >(
      `resellers?${filter({ id: `eq.${resellerClients[0].reseller_id}`, status: 'eq.active', select: 'id,commission_rate,total_earned,pending_payout', limit: '1' })}`,
    ).catch(() => []);
    const reseller = resellers[0];
    if (reseller) {
      const rawCommission = +(
        overageRevenueUsd * (reseller.commission_rate || 0.2)
      ).toFixed(2);
      const { cappedCommissionUsd, wasCapped } =
        applySmsOverageCommissionSafeguard(overageRevenueUsd, rawCommission);
      if (cappedCommissionUsd > 0) {
        ...
        if (wasCapped)
          console.warn(
            `[sms-billing] overage commission capped for reseller ${reseller.id} on invoice ${invoiceId}`,
          );
      }
    }
  }
...
export async function reconcileOverages(): Promise<{ processed: number }> {
  ...
  >(
    `sms_billing_periods?${filter({ ends_at: `lt.${new Date().toISOString()}`, stripe_invoice_item_id: 'is.null', overage_micros: 'gt.0', limit: '20' })}`,
  );
  for (const p of periods) {
    ...
    const item = await stripe(
      ...
      'POST',
      {
        ...
        amount: String(Math.ceil(p.overage_micros / 10000)),
        currency: 'usd',
        description: 'BuildMyBot SMS overage segments',
        'metadata[sms_period]': p.invoice_id,
      },
      `sms-overage-item:${p.invoice_id}`,
    );
    ...
    const invoice = await stripe(
      ...
      `sms-overage-invoice:${p.invoice_id}`,
    );
    await stripe(
      `/invoiceitems/${encodeURIComponent(item.id)}`,
      'POST',
      { invoice: invoice.id },
      `sms-overage-attach:${p.invoice_id}`,
    );
    await db(
      `sms_billing_periods?${filter({ invoice_id: `eq.${p.invoice_id}` })}`,
      'PATCH',
      { stripe_invoice_item_id: item.id },
    );
    await recordSmsOverageCommission(
      subscription,
      p.overage_micros,
      p.invoice_id,
    ).catch((err: any) =>
      console.error('[sms-billing] overage commission error:', err?.message),
    );
  }
