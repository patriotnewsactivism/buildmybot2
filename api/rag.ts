...
export async function scrapeUrlFirecrawl(url: string): Promise<string> {
	...
	const basic = () => scrapeUrl(url);
	if (!FIRECRAWL_API_KEY) return basic();
	try {
		...
		const htmlText = data?.data?.html
			? String(data.data.html)
					...
					.trim()
			: "";
		const best = [md, htmlText].sort((a, b) => b.length - a.length)[0] || "";
		if (best.length >= 50) return best.slice(0, 100000);
