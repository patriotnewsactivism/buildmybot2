...
import { RESET_PASSWORD_PATH, VERIFY_EMAIL_PATH } from './shared/auth-links';
import {
  type Bot as BotType,
  type Conversation,
  type Lead,
...
function App() {
  ...
  // Finish boot once auth has settled (not a fixed timer) so hard-loads of
  // /app* don't race past session restore into the public homepage (#118).
  useEffect(() => {
    if (!authLoading) setIsBooting(false);
  }, [authLoading]);
