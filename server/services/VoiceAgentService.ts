...
import { env } from '../env';
import { db } from '../db';
import { bots, users, conversations } from '../../shared/schema';
...
export class VoiceAgentService {
        ...
        switch (msg.event) {
          case 'start':
            ...
            session = {
              userId,
              botId: bot?.id,
              voiceId: msg.start.customParameters?.voiceId || 'a0e99841-438c-4a64-b679-ae501e7d6091',
              introMessage: msg.start.customParameters?.introMessage || 'Hello!',
              streamSid: msg.start.streamSid,
              systemPrompt: bot?.systemPrompt || 'You are a helpful AI assistant.',
              conversationId: uuidv4(),
            };

            console.log(`Voice session started. User: ${session.userId}, Bot: ${session.botId}, Stream: ${session.streamSid}`);
            ...
            if (session.introMessage) {
                ...
                setTimeout(() => {
                    if (cartesiaWs && cartesiaWs.readyState === WebSocket.OPEN) {
                        this.sendToCartesia(cartesiaWs, session!.introMessage, session!.voiceId);
                    }
                }, 500);
            }
            break;
  ...
  private async appendToConversation(conversationId: string, role: 'user' | 'assistant', content: string) {
      try {
  ...
  private initOpenAI(twilioWs: WebSocket, session: VoiceAgentSession, onTextDelta: (text: string) => void): WebSocket {
    ...
    ws.on('message', (data) => {
      ...
      if (response.type === 'conversation.item.input_audio_transcription.completed') {
          // Log user message
          this.appendToConversation(session.conversationId, 'user', response.transcript);
      }

      if (response.type === 'response.text.done') {
          // Log assistant message
          this.appendToConversation(session.conversationId, 'assistant', response.text);
      }
  ...
  private initCartesia(twilioWs: WebSocket, session: VoiceAgentSession): WebSocket {
