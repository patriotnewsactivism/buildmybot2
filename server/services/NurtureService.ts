...
export class NurtureService {
  async createSequence(organizationId: string, name: string, description: string, triggerType: string) {
    const [sequence] = await db.insert(nurtureSequences).values({
      id: uuidv4(),
      organizationId,
      name,
      description,
  ...
  async addStep(sequenceId: string, stepOrder: number, delayHours: number, actionType: string, actionConfig: any) {
    const [step] = await db.insert(nurtureSteps).values({
      id: uuidv4(),
      sequenceId,
      stepOrder,
      delayHours,
  ...
  async enrollLead(leadId: string, sequenceId: string) {
    ...
    const existing = await db.select().from(nurtureEnrollments).where(
      and(
    ...
    const steps = await db.select().from(nurtureSteps)
  ...
  private async processEnrollmentStep(enrollment: any) {
    ...
    const [step] = await db.select().from(nurtureSteps).where(
      and(
    ...
    if (!step) {
      ...
      await db.update(nurtureEnrollments)
    ...
    // Move to next step
    const [nextStep] = await db.select().from(nurtureSteps)
      .where(
        and(
          eq(nurtureSteps.sequenceId, enrollment.sequenceId),
    ...
    if (nextStep) {
      ...
      await db.update(nurtureEnrollments).set({
