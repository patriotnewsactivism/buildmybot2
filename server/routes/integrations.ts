...
router.get('/providers', ...apiAuthStack, async (req: Request, res: Response) => {
  try {
...
router.post('/connect', ...apiAuthStack, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    ...
    if (!providerId || !config) {
      return res.status(400).json({ error: 'Provider ID and config are required' });
    }
    ...
    res.status(400).json({ error: error.message || 'Failed to connect integration' });
  }
});

router.post('/disconnect', ...apiAuthStack, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
