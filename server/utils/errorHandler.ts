...
export const UserFriendlyMessages: Record<string, string> = {
  ...
  [ErrorCodes.PAYMENT_REQUIRED]: 'Please upgrade your plan to access this feature.',
  [ErrorCodes.EXTERNAL_SERVICE_ERROR]: 'An external service is temporarily unavailable. Please try again later.',
...
function getStatusCodeFromError(error: Error): number {
  ...
  if (message.includes('forbidden') || message.includes('not authorized') || message.includes('access denied')) {
    return 403;
  }
...
export const authenticationError = (message = 'Authentication required'): AppError =>
  createError(401, message, ErrorCodes.AUTHENTICATION_ERROR);
...
export const externalServiceError = (service: string): AppError =>
  createError(503, `${service} is temporarily unavailable`, ErrorCodes.EXTERNAL_SERVICE_ERROR);
