...
const ErrorRecoveryDashboard: React.FC = () => {
    ...
    try {
      ...
      if (result.success) {
        setErrorLogs((prev) =>
          prev.map((l) =>
            l.id === log.id
              ? { ...l, resolved: true, autoFixApplied: true }
              : l,
          ),
        );
      }
  ...
  const handleResolve = async (logId: string) => {
    try {
      ...
      setErrorLogs((prev) =>
        prev.map((l) => (l.id === logId ? { ...l, resolved: true } : l)),
      );
    } catch (err) {
  ...
  const getSeverityCount = (severity: string) => {
    return errorLogs.filter((l) => l.severity === severity && !l.resolved)
      .length;
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-4 lg:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div
          className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium min-h-[44px] min-w-[44px]"
          style={{
              ...
              >
                <div className="p-3 sm:p-4">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3">
                    ...
                    <div
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium border min-h-[32px] sm:min-h-[28px] ${
  ...
  const errorTypes = [
    {
      ...
      fix: 'Increase timeout limit or optimize API response',
    },
    {
      type: 'RATE_LIMIT',
      message: 'Rate limit exceeded for OpenAI API',
      severity: 'medium' as const,
      fix: 'Implement exponential backoff or increase rate limit',
    },
    {
      type: 'VOICE_SYNTHESIS',
      message: 'Voice synthesis failed: invalid audio format',
      severity: 'critical' as const,
      fix: 'Restart voice agent service',
    },
    {
      type: 'KNOWLEDGE_RETRIEVAL',
      message: 'Failed to retrieve knowledge base chunks',
      severity: 'high' as const,
      fix: 'Re-index knowledge base',
    },
    {
    ...
    },
    {
      ...
      severity: 'medium' as const,
      fix: 'Retry with different prompt',
    },
    {
      type: 'WEBHOOK_DELIVERY',
      message: 'Webhook delivery failed after 3 retries',
      severity: 'low' as const,
      fix: 'Check webhook endpoint availability',
    },
    {
      type: 'AUTH_TOKEN',
      message: 'Authentication token expired',
      severity: 'high' as const,
      fix: 'Refresh authentication token',
    },
  ];
