...
import { type LogLine, TerminalConsole } from '../UI/TerminalConsole';
...
export const AdminDashboardV2: React.FC<AdminDashboardV2Props> = ({
  ...
  const systemStatus = errorMetrics
    ? 'warning'
    : loadingMetrics
      ? 'processing'
      : 'online';
  ...
  const ingestionItems = useMemo(
    () => [
      ...
      },
      {
        ...
        status:
          (metrics?.supportTicketsOpen ?? 0) > 0 ? 'processing' : 'online',
      },
    ],
    [metrics],
  ) as {
        ...
        <div className="mb-4 mt-4 flex flex-col gap-3 sm:mb-5 sm:mt-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-[11px] text-console-muted">
              Overview of system health, user activity &amp; financial
              performance
            </p>
          </div>
          <nav
                ...
                <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
                  ...
                  <HudMetric
                    ...
                    value={
                      loadingMetrics
                        ? '—'
                        : (metrics?.supportTicketsOpen?.toLocaleString() ??
                          'N/A')
                    }
                    accent={
