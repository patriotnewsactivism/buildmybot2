...
export const SmsAccountSettings: React.FC = () => {
  ...
  const mintBookingSecret = async () => {
    ...
    try {
      ...
      setNotice('Booking secret generated — copy it now; it won’t be shown again.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not mint booking secret');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-lg border border-gray-200 bg-white p-5 space-y-4">
        ...
        {account?.ready ? (
          ...
          <p className="text-xs text-amber-700">
            Sender not ready yet — finish registration / approval before activating programs.
          </p>
        )}
        {!launchEnabled && (
          <p className="text-xs text-amber-700">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="block text-sm">
            <span className="font-medium text-gray-700">Quiet start (hour)</span>
            <input
              type="number"
              min={9}
              max={19}
          ...
          <label className="flex items-center gap-2 text-sm sm:col-span-2">
            ...
            />
            <span className="font-medium text-gray-700">AI replies enabled</span>
          </label>
          <label className="block text-sm sm:col-span-2">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-5 space-y-3">
        <h3 className="text-base font-semibold text-gray-900">Booking webhook</h3>
        <button
          type="button"
          disabled={busy}
