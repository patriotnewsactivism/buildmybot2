 ...
 * API Configuration
 *
 * In production, the frontend and serverless API run on the same Vercel
 * deployment, so relative /api/ paths avoid cross-origin requests.
 *
 ...
 * Vercel routes /api/* to this repository's serverless functions.
 */
...
function getApiBase(): string {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (!isLocalDev) {
      // Production: always use the same-origin Vercel API.
      return '/api';
    }
  }
