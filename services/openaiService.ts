...
export const generateBotResponseDemo = async (
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			console.error(
				`Chat attempt ${attempt + 1} failed${isAbort ? " (timeout)" : ""}:`,
				error,
			);
...
export const scrapeWebsiteContent = async (url: string): Promise<string> => {
	...
	// Summarize separately so summarize failures are not masked as scrape failures (#120).
	try {
		...
		if (!response.ok) {
			const err = await response.json().catch(() => ({}));
			throw new Error(err.error || "Failed to summarize content.");
		}
		const data = await response.json();
		return data.response || rawText.substring(0, 1000);
	} catch (error: any) {
		console.error("Summarize Error:", error);
		return `Scraped content (summary unavailable): ${rawText.substring(0, 1000)}`;
	}
};
