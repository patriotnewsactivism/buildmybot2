...
export const timezoneSchema = z
  .string()
  .min(1)
  .refine(validTimezone, 'Use a valid timezone, such as America/Chicago');
...
export function nextSendTime(
  ...
  const next = new Date(
    Date.UTC(p.year, p.month - 1, p.day + (p.hour >= end ? 1 : 0)),
  );
  return localInstant(
    next.getUTCFullYear(),
    next.getUTCMonth() + 1,
...
export function renderSms(
  ...
  return template
    .replace(/\{\{\s*([a-z_]+)\s*\}\}/g, (_, key: string) => fields[key] || '')
    .trim();
}
...
export const contactSchema = z
  .object({
    ...
    consents: z.array(z.enum(PURPOSES)).default([]),
    consentSource: z.string().trim().min(1).max(500),
  })
  .refine(
    (c) =>
      (!c.birthMonth && !c.birthDay) ||
      (!!c.birthMonth &&
        !!c.birthDay &&
        validBirthday(c.birthMonth, c.birthDay)),
    'Invalid birthday',
  );
...
export const programSchema = z
  .object({
    ...
    prize: z.string().trim().max(500).optional(),
    rulesUrl: z.url().optional(),
    entryUrl: z.url().optional(),
    eligibility: z.string().trim().max(2000).optional(),
    winnerCount: z.number().int().min(1).max(100).default(1),
    winnerText: textSchema.optional(),
    confirmationText: textSchema.optional(),
  })
  .superRefine((p, ctx) => {
    if (p.keyword && SYSTEM_KEYWORDS.has(p.keyword))
      ctx.addIssue({
        code: 'custom',
    ...
    if (
      ['keyword', 'sequence', 'contest', 'birthday'].includes(p.kind) &&
      !p.keyword
    )
      ctx.addIssue({
        code: 'custom',
    ...
    if (p.kind === 'contest') {
      for (const field of [
        ...
        if (!p[field])
          ctx.addIssue({
            code: 'custom',
            message: `${field} is required for contests`,
            path: [field],
          });
      }
      if (p.opensAt && p.closesAt && p.opensAt >= p.closesAt)
        ctx.addIssue({
          code: 'custom',
...
export const appointmentSchema = z.object({
  ...
  text: textSchema.default(
    'Reminder: your appointment with {{business}} is {{appointment_time}}. Reply CONFIRM to confirm or RESCHEDULE for help. Reply STOP to opt out.',
  ),
});
