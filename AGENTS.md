> **⚠️ 2026-09-05 correction:** This file describes a `server/` Express app (routes, migrations, seeds, services). That directory **does not exist anywhere in this repo's git history** — confirmed via `git log --diff-filter=D -- server/` (empty) and a live GitHub API check. The actual production stack is a real Node/Express container (`server.ts` + `Dockerfile`, mounting `api/*.ts` handlers such as `api/gateway.ts`, `api/cron/`, `api/ai-team/`, `api/rag.ts`) deployed to **Railway** (primary) with **Google Cloud Run** as fallback, fronted by Cloudflare Pages — not Vercel serverless functions (an earlier version of this note claimed that; it's now stale too, and Vercel is not the production authority here — see `.vercelignore`). `package.json`'s `start`/`server` scripts still reference the phantom `server/index.ts` and will fail if run. Treat every `server/` reference below as historical/aspirational, not current. For the real structure, see `CLAUDE.md` and `DEPLOYMENT.md`.
>
